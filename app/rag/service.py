...
class RAGService:
    ...
    async def build_context_for_lesson_question(
        ...
        if requested_name and not matches:
            ...
            attribution_notice = (
                f"[ПОДТВЕРЖДЁННЫЙ СПИКЕР] Все найденные фрагменты размечены как речь "
                f"спикера «{requested_name}». Отвечай только по этим фрагментам. "
                "Если пользователь просит цитаты, приводи только дословный текст из них и не выдумывай таймкоды."
            )
            ...
            attribution_notice = (
                "[ОБЩЕЕ ОБСУЖДЕНИЕ] Найденные фрагменты относятся к анонимным или неразмеченным репликам. "
                "Содержательно ответь на вопрос по этим фрагментам, но не приписывай мысли конкретным людям "
                "и не отказывайся от ответа только потому, что авторы реплик не установлены."
            )
